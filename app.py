...
@app.post("/reset_task")
def reset_task(task_id: str = Query("data_fetch")):
...
@app.post("/step_task")
def step_task(action: ToolChainAction, task_id: str = Query("data_fetch")):
...
@app.get("/state_task")
def state_task(task_id: str = Query("data_fetch")):
    return JSONResponse(content=_get_or_create(task_id).state().model_dump())

@app.post("/grader")
def grader(task_id: str = Query("data_fetch")):
    env = _get_or_create(task_id)
